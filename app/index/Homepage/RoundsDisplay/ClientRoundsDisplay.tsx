...
import { RoundCard, Button, Badge } from "@/components/ui/primitives";
...
export const ClientRoundsDisplay = ({ rounds, currentRoundId, isVotingPhase }: Props) => {
  ...
  return (
    <div className="py-16 bg-gray-900/20 rounded-xl">
              ...
              return (
                ...
                >
                  <Link href={`round/${round.roundId}`}>
                    <div className={`p-6 rounded-lg border ${isCurrent ? 'bg-gray-800/60 border-[#e2e240]' : 'bg-gray-900/40 border-gray-800 hover:border-gray-700'} transition-all duration-200`}>
                      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                        <div>
                          {isCurrent && (
                            <Badge className="mb-2 bg-[#e2e240] text-gray-900">
                              Current Round
                            </Badge>
                          )}
                          
                          <h3 className="text-xl md:text-2xl font-bold text-white mb-1 flex items-center gap-2">
                            <Music className="h-5 w-5 text-[#e2e240]" />
                            {isCurrent && isVotingPhase 
                              ? "Song selection underway" 
                              : <>{round.title} <span className="text-gray-400">by</span> {round.artist}</>}
                          </h3>
                        ...
                        <div className="flex gap-4">
                          {round.signupCount !== undefined && (
                            <div className="flex flex-col items-center px-3 py-2 bg-gray-800/50 rounded">
                              <div className="flex items-center gap-1 text-gray-300 text-sm">
                                <Users className="h-4 w-4 text-[#e2e240]" />
                                Participants
                              </div>
                              <span className="text-xl font-bold text-white">{round.signupCount}</span>
                            </div>
                          )}
                          ...
                          {round.submissionCount !== undefined && !isCurrent && (
                            <div className="flex flex-col items-center px-3 py-2 bg-gray-800/50 rounded">
                              ...
                              <span className="text-xl font-bold text-white">{round.submissionCount}</span>
                            </div>
                          )}
                        </div>
                      </div>
