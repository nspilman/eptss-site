/**
 * Build atjam lexicon records from a flattened EPTSS round export — a pure
 * function (no DB, no network) so the mapping is easy to inspect and test.
 * This is the ONE truth for "how an EPTSS round becomes an at.atjam.round":
 * the historical backfill (packages/scripts) and the live publisher
 * (@eptss/rounds) both flow through it.
 *
 * Produces:
 ...
 */
import { eptssRoundRkey, eptssSubmissionRkey } from "./eptss-rkey";
...
/** A round, flattened out of Postgres — the input contract for transformRound. */
