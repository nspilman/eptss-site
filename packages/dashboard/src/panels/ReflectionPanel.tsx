...
/**
 * Reflection Panel - Shows user reflections
 * Priority: tertiary (collapsible, below fold)
 */
export function ReflectionPanel({ data }: PanelProps<ReflectionData>) {
  if (!data) {
    return null;
  }

  const { roundSlug, reflections } = data;
      ...
      {reflections.length > 0 ? (
        <div className="space-y-4">
          ...
          <div className="space-y-3">
            {reflections.map((reflection) => (
              <div
                key={reflection.id}
                className="p-4 bg-[var(--color-gray-900-40)] rounded-lg border border-[var(--color-gray-800)] hover:border-[var(--color-accent-primary)] transition-colors"
              >
          ...
          <Link
            ...
            className="inline-flex items-center gap-2 px-6 py-3 bg-[var(--color-accent-primary)] text-white font-medium rounded-lg hover:bg-[var(--color-accent-secondary)] transition-colors"
          >
            <svg
              className="w-5 h-5"
              fill="none"
        ...
        <div className="space-y-4">
          <p className="text-[var(--color-gray-300)]">
            Start your journey by reflecting on your goals, expectations, and
            approach for this round.
          </p>

          <Link
            href={`/round/${roundSlug}/create-reflection`}
            className="inline-flex items-center gap-2 px-6 py-3 bg-[var(--color-accent-primary)] text-black font-medium rounded-lg hover:bg-[var(--color-accent-secondary)] transition-colors"
          >
            <svg
              className="w-5 h-5"
              fill="none"
