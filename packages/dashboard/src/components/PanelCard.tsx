...
export function PanelCard({
  ...
  if (variant === 'sidebar') {
    return (
      <div className={`relative overflow-hidden rounded-lg bg-background-primary/60 backdrop-blur-sm border border-gray-800 p-4 ${className}`}>
        {children}
      </div>
    );
  }
  ...
  return (
    <div className={`relative overflow-hidden rounded-lg border border-[var(--color-gray-800)] bg-[var(--color-gray-900-40)] backdrop-blur-sm p-5 lg:p-6 ${className}`}>
      {children}
    </div>
  );
}
